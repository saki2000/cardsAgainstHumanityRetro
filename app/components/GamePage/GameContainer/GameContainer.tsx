...
export default function GameContainer() {
  const { slots, playCard } = useCardDeckStore();
    ...
    <DndContext onDragEnd={handleDragEnd}>
      <div className="flex flex-1 flex-col items-center justify-center p-8 space-y-12">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 ">
          <TableSlot id="slot1">
            {slots.slot1 ? (
              <Card id={slots.slot1.id} text={slots.slot1.text} />
            ) : (
          ...
          <TableSlot id="slot2">
            {slots.slot2 ? (
              <Card id={slots.slot2.id} text={slots.slot2.text} />
            ) : (
              <span className="text-gray-400">Card Slot</span>
            )}
          </TableSlot>
          <TableSlot id="slot3">
            {slots.slot3 ? (
              <Card id={slots.slot3.id} text={slots.slot3.text} />
            ) : (
              <span className="text-gray-400">Card Slot</span>
            )}
          </TableSlot>
